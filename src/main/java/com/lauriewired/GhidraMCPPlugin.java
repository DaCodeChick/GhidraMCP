 ...
 */
@PluginInfo(status = PluginStatus.RELEASED, packageName = ghidra.app.DeveloperPluginPackage.NAME, category = "Developer", shortDescription = "HTTP server plugin", description = "Starts an embedded HTTP server to expose program data. Port configurable via Tool Options.")
public class GhidraMCPPlugin extends Plugin {
	 ...
	 * 
	 * This method scans the program's function manager to collect all function
	 * names
	 ...
	 * @return Newline-separated string of function names, or error message if no
	 *         program loaded
	 ...
	 * Ghidra's DecompInterface to generate readable C-like pseudocode. The
	 * decompilation
	 * process includes:
	 * <ul>
	 ...
	 * </ol>
	 * 
	 * <b>Thread Safety:</b> This method uses SwingUtilities.invokeAndWait() to
	 * ensure
	 * all Ghidra API calls occur on the Event Dispatch Thread, preventing race
	 * conditions
	 * and maintaining data integrity.
	 * 
	 * <b>Transaction Management:</b> The rename operation is wrapped in a Ghidra
	 * transaction
	 * to ensure atomicity - either the rename succeeds completely or is fully
	 * reverted.
	 * 
	 ...
	 *         {@code false} if no program is loaded, function not found, or rename
	 *         failed
	 ...
	 * 
	 * This immutable class encapsulates the outcome of attempting to set a
	 * function's
	 * prototype signature, including both success/failure status and detailed error
	 * information for debugging purposes.
	 * 
	 * <p>
	 * The class is designed to provide comprehensive feedback about prototype
	 ...
	 * 
	 * This method extracts query parameters from the URL (the portion after the '?'
	 * character)
	 * and decodes them using UTF-8 encoding. It handles URL-encoded parameter
	 * values and
	 ...
	 * }</pre>
	 * 
	 * @param exchange The HTTP exchange containing the request with query
	 *                 parameters
	 * @return A map of parameter names to their decoded values. Empty map if no
	 *         parameters exist.
	 ...
	 * <li>If offset + limit exceeds list size, returns items from offset to
	 * end</li>
