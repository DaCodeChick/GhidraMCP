...
/**
 * Unit tests specifically for endpoint registration and HTTP server
 * functionality.
 *
 * This test class focuses on validating that all endpoints defined in the
 * Ghidra MCP are properly registered with the HTTP server.
 */
public class EndpointRegistrationTest extends TestCase {
	...
	/**
	 * Setup the HTTP client before each test
	 */
	@Override
	...
	private boolean isEndpointRegistered(String endpoint) {
		try {
			...
			// It may return 400 (bad request) if parameters are missing, but that means
			// it's registered
	...
	public void testSearchAnalysisEndpointsRegistered() {
		...
		for (Map.Entry<String, String> entry : searchEndpoints.entrySet()) {
			assertTrue(
					"Search/Analysis endpoint '" + entry.getKey() + "' (" + entry.getValue() + ") should be registered",
					isEndpointRegistered(entry.getKey()));
		}
	}
	...
	public void testCrossReferenceEndpointsRegistered() {
		...
		for (Map.Entry<String, String> entry : xrefEndpoints.entrySet()) {
			assertTrue(
					"Cross-reference endpoint '" + entry.getKey() + "' (" + entry.getValue() + ") should be registered",
					isEndpointRegistered(entry.getKey()));
		}
	}
	...
	public void testCurrentStateEndpointsRegistered() {
		...
		for (Map.Entry<String, String> entry : stateEndpoints.entrySet()) {
			assertTrue(
					"Current state endpoint '" + entry.getKey() + "' (" + entry.getValue() + ") should be registered",
					isEndpointRegistered(entry.getKey()));
		}
	}
	...
	public void testAdvancedEndpointsRegistration() {
		...
		// Document the current state - this test should eventually pass when all
		// endpoints are fixed
		...
		// assertTrue("All advanced endpoints should be registered", registeredCount ==
		// totalCount);
	}
	...
	/**
	 * Test endpoint consistency - all endpoints from Java source should be
	 * registered
	 * This test verifies that the createContext() calls in the Java code are
	 * working
	 */
